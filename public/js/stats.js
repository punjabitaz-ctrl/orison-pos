...
/* Held for a repair, not earned. Money moves, but revenue does not. */
export const DEPOSIT_KINDS = ['deposit', 'deposit_refund'];
...
export function signedNet(tx) {
  ...
  return (k === 'refund' || k === 'tradein' || k === 'supplier_payment' || k === 'wages' || k === 'opex' || CASH_OUT.includes(k)) ? -v : v;
}
...
export function dayTotals(txs, key) {
  const out = { key, net: 0, sales: 0, refunds: 0, payouts: 0, pickups: 0, expenses: 0, cashOut: 0, collections: 0, deposits: 0, depositRefunds: 0, tradeIns: 0, supplierPayments: 0, wages: 0, runningCosts: 0, count: 0, units: 0, gp: 0, tickets: 0 };
  for (const t of txs || []) {
    ...
    else if (k === 'supplier_payment') out.supplierPayments += v;
    else if (k === 'wages') out.wages += v;
    else if (k === 'opex') out.runningCosts += v;
    out.net += signedNet(t);
  }
