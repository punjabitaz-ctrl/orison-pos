...
| Hand somebody a page | Press **PDF** instead of *Export CSV* — the books (profit and loss plus trial balance), reports, stock health, running costs, a pay run, customers, purchases, repairs or products. It opens the print window; choose *Save as PDF*. If nothing opens, allow pop-ups for the site. |
| Get the numbers out | Every screen that shows data has **Export CSV** — customers, purchases, repairs, products, trade-ins, the time clock, a pay run, running costs, reports, the sales report, stock health, the books and the audit log. Files are named `orison-<what>-<date>.csv` and their column headers are always English, so months can be joined together. |
