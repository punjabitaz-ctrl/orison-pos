...
```js
  ...
  check('a part can be fitted to a ticket', fit.ok === true, JSON.stringify(fit.data));
  ...
  check('a serialized part can be fitted', fitSer.ok === true, JSON.stringify(fitSer.data));
...
```js
  ...
  check('labour can be added', lab.ok === true, JSON.stringify(lab.data));
